// components/products/GiftSomeone.tsx
"use client";
...
type Props = {
  products: Product[];
  currentGender?: Gender; // "male" | "female"
  excludeSlug?: string;
};
...
export default function GiftSomeone({ products, currentGender, excludeSlug }: Props) {
  const opposite: Gender | undefined =
    currentGender === "male" ? "female" : currentGender === "female" ? "male" : undefined;

  const picks = products
    .filter((p) => (excludeSlug ? p.slug !== excludeSlug : true))
    .filter((p) => (opposite ? p.gender === opposite : true))
    .slice(0, 2);
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 sm:p-8 backdrop-blur-xl">
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs tracking-[0.32em] text-white/45">OU PRESENTEIE ALGUÉM</p>
        <p className="text-xs tracking-[0.22em] text-white/45">
          {opposite === "female" ? "FEMININO" : "MASCULINO"}
        </p>
      </div>

      <p className="mt-3 text-sm text-white/60">
        Um presente com presença. Escolha uma peça do outro lado do drop.
      </p>
